 function R = NoiseExtractCBM3D( X, sigma )
%function R = NoiseExtractCBM3D( X, sigma )
%
% The function computes the residue through 
%  the  BM3D algorithm described in [1] for grayscale images, and 
%  the CBM3D algorithm described in [2] for color images (denoising in YUV space).
%
%
% INPUT PARAMETERS:
%     |  parametro  |  tipo  | commento
%  ---|-------------|--------|-------------------------------
%   1 |      X      |  image | input image
%   2 |    sigma    | scalar | standard deviation of noise
%
%
% REFERENCES:
% [1] Foi et al, Image denoising by sparse 3D transform-domain collaborative filtering
% [2] Foi et al, Color image denoising via sparse 3D collaborative filtering with 
%     grouping constraint in luminance-chrominance space
%

% controllo su sigma
if length(sigma) ~= 1
    error('"sigma" deve essere uno scalare');
end
X = double(X);
% esegui il denoising
if size(X,3) == 3              %%% immagini a colori

    [void Y_norm] = actual_CBM3D( 1, X, sigma, 'lc' );
    R = X - 255 * Y_norm;
...
function [PSNR, yRGB_est] = actual_CBM3D(yRGB, zRGB, sigma, profile, print_to_screen, colorspace)
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Copyright (c) 2007 Tampere University of Technology. All rights reserved.

function [RP,LP] = getFingerprint_Binghamton(Images, getResidue) 
% function [RP,LP] = new_pattern_Binghamton(Images, getResidue) 
% 
% Extracts and averages noise from all Images and outputs a camera fingerprint 
%
%  INPUT PARAMETERS:
%    Images      list of images to process (obtained through dir command)
%                these images have to be from the same camera and the same size
%    getResidue  function handler to get residue based on a denoising algorithm
%
% OUTPUT PARAMETERS:
%    RP          reference pattern - estimate of PRNU
%    LP          linear pattern structured data
%
% This function is a modified version of the "getFingerprint.m" function of 
%   DDE Lab, Binghamton University, NY.
% The original version can be downloaded from:
%    http://dde.binghamton.edu/download/camera_fingerprint/
%
% -------------------------------------------------------------------------
% ------------------   Copyright of getFingerprint.m  ---------------------
% -------------------------------------------------------------------------
...
for i=1:database_size
    ...
    ImNoise = getResidue(X); 
    
    for j=1:3    
        Inten = single(IntenScale_Binghamton(X(:,:,j))).*Saturation_Binghamton(X(:,:,j));    % zeros for saturated pixels
        RPsum{j} = RPsum{j}+ImNoise(:,:,j).*Inten;   	% weighted average of ImNoise (weighted by Inten)
        NN{j} = NN{j} + Inten.^2;
    end
